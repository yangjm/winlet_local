...
var win$ = function() {
	...
	var WinletJSEngine = {
		...
		form : {
			...
			validateClearAll : function(container) {
				var $container = container instanceof jQuery ? container
						: $(container);
		...
		startsWith : function(str, w) {
			return str != null && typeof str == 'string' && w
					&& str.slice(0, w.length) == w;
		},

		endsWith : function(str, w) {
			return str != null && typeof str == 'string' && w
					&& str.slice(-w.length) == w;
		},
			...
			if (WinletJSEngine.winletDomain) {
				if (typeof (sessionStorage) !== "undefined") {
					...
					return decodeURIComponent(document.cookie.replace(
							new RegExp("(?:(?:^|.*;)\\s*"
									+ encodeURIComponent("WINLET_SESSION_ID")
											.replace(/[\-\.\+\*]/g, "\\$&")
									+ "\\s*\\=\\s*([^;]*).*$)|^.*$"), "$1"))
							|| null;
				}
		...
		isRootWinlet : function($winlet) {
			return $winlet.data("winlet-url") != null
					&& $winlet.parent().closest("div[data-winlet-url]").length == 0;
		},
		...
		traceToWinlet : function($container) {
			...
			return WinletJSEngine.traceToWinlet($('div[data-winlet-id="'
					+ $container.data("winlet-src-id") + '"]'));
		},
		...
		getWinletUrl : function($container, url) {
			...
			case 1:
				...
				break;
			case 2:
				return WinletJSEngine.getContextRoot($winlet)
						+ url.substring(1);
				break;
			}
		...
		getFullUrl : function(url) {
			if (WinletJSEngine.winletDomain) {
				...
				var idx = url.indexOf("?");
				if (idx <= 0)
					return WinletJSEngine.winletDomain + url + ";jsessionid="
							+ sid;

				return WinletJSEngine.winletDomain + url.substr(0, idx)
						+ ";jsessionid=" + sid + url.substr(idx);
			}
		...
		getViewport : function() {
			...
			rect.left = (window.pageXOffset || doc.scrollLeft)
					- (doc.clientLeft || 0);
			rect.top = (window.pageYOffset || doc.scrollTop)
					- (doc.clientTop || 0);
		...
		showLoading : function($container, dialog, nodelay) {
			try {
				...
				if (WinletJSEngine.ImgLoading.src != null
					...
					$container[0].loading = $("<div style='z-index:100000;position:absolute;background:url("
							+ WinletJSEngine.ImgBg.src
							+ ");left:"
							+ rect.left
							+ "px;top:"
		...
		procWinFunc : function(cont, $container) {
			...
			return cont.replace(WinletJSEngine.reWinPost,
		...
		loadContent : function($container, focus, pageRefresh,
			...
			if (!pageRefresh && loadWhenHashChanged) { // 只有hash参数发生变化时才重新加载
				...
				if (hashParams != null
						&& savedParams != null
						&& Object.keys(hashParams).length == Object
					...
					for ( var key in hashParams) {
						if (Object.prototype.toString.call(hashParams[key]) === '[object Array]') {
							if (!Object.prototype.toString
								...
								same = false;
							else if ($(hashParams[key]).not(savedParams[key]).length != 0
									|| $(savedParams[key]).not(hashParams[key]).length != 0)
								same = false;
			...
			$.ajax({
				type : 'POST',
				url : WinletJSEngine.getFullUrl(WinletJSEngine
						.getWinletUrl($winlet)),
		...
		isInt : function(n) {
			return n != undefined && n != null && Number(n) === n
					&& n % 1 === 0;
		},
		...
		setup : function(settings) {
			if (settings) {
				...
				if (settings.hashchange
						&& typeof settings.hashchange === 'function')
					WinletJSEngine.hashchange = settings.hashchange;
				if (settings.afterload
						&& typeof settings.afterload === 'function')
				...
				.each(function() {
					...
					if (!$form.attr("action").match(WinletJSEngine.reAction)) { // 服务器端没有用<win:form>,
						...
						this.winletAction = WinletJSEngine
								.getWinletUrl(settings.container)
								+ "?_a=" + $form.attr("action");
					...
					this.winsubmit = function(state) {
						...
						var $target = settings.target ? settings.target
								: $container;

						if ($form.attr("enctype") == "multipart/form-data") {
							form.action = form.winletAction + "&"
					...
					this.ajaxValidate = function(input, name, value) {
						...
						$.ajax({
							type : 'POST',
							url : WinletJSEngine.getFullUrl(form.winletAction),
					...
					if (settings.focus) {
						var inp = $form.find(':input[name="' + settings.focus
								+ '"]');
						inp.select();
						inp.focus();
					}
	...
	$(window).hashchange(function() {
		...
		if (WinletJSEngine.analytic)
			WinletJSEngine.analytic('hashChanged');
	...
	return {
		...
		_post : function(element, container, action) {
			...
			if (idx > 0) { // 调用其他Winlet的Action
				...
				$container = $('div[data-winlet-url^="'
						+ WinletJSEngine.getWinletUrl($container, winlet)
						+ '"]');
				if ($container.length == 0)
					return dfd.reject().promise();
			...
			if (container != null) {
				...
				if ($targetContainer.data("winlet-id") == null
						|| $targetContainer.data("winlet-id") == '')
		...
		_include : function(element, container, url) {
			...
			if ($target.data("winlet-id") == null
					|| $target.data("winlet-id") == '')
			...
			return WinletJSEngine.loadContent($target, false);
		},
		...
		_get : function(element, param) {
			...
			if (reload)
				arr.push(WinletJSEngine.loadContent($container, focus
						&& !focusUpdate));
			if (update)
		...
		_wait : function(element, selector, maxwait) {
			...
			(function() {
				if ($container.find(selector) == undefined) {
					if (maxwait != null
							&& (new Date()).getTime() - start > maxwait)
						dfd.reject();
					else
